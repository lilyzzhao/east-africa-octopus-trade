...
dataPackage$commit()
filepath <- read.csv("data/Octopus-Middlemen-Data.csv")
...
```{r}
schema <- tableschema.r::infer(filepath) ##inferring schema doens't work in R but does on gui. 
...
##title vs. name for each resouces the same as metadata name and titles?  plus data tabular data profile # same as settings bar. 
